...
//        	Debug.Log ("number"+i+" is" + edge_list [i*2] + "and"+ edge_list [i*2+1]);
    ...
    private Vector3[] GravityAccelerate(Vector3[] vertices)
    {
        int i = 0;
    ...
    // Basic setup
    // 1) access mesh using the Get Component function 
    // 2) accelerate velocity of each vertex (except for vertex 0 and 10) using the gravity;
    // 3) damp each vertex velocity using a constant: vi = damping 
    // 4) update vertex positions in the mesh vertex array using the velocity.
    //    By doing this, you should see the cloth piece dropping (except for two vertices: 0 and 10).
    //    Since the gravity is the only force and it is constant, you can apply the acceleration on the velocity directly without defining the forces.
}
